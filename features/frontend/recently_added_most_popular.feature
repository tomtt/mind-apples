...
Feature: As a social butterfly I would like to see the most popular and recently added sections of the home page
  ...
  Scenario: As a curious cat I can see the 'Most recent' section
    Given a user exists with login: "andyman", email: "andy@overlord.com"
    And person "andy" exists with user: that user, name: "Andy Gibson"
    And a user exists with login: "miss_bashful", email: "miss@bashful.com"
    And person "anonym" exists with user: that user, name: ""
    And person "anonym2" exists with page_code: "asdasd", email: "annonym@annonym.com"
    And person "anonym3" exists with page_code: "bcdefr", email: "annony3m@annonym.com"
    ...
    Then I should see homepage "Most recent" section
  ...
  Scenario: As a curious can't see name of user at mindapples if his profile is private
    Given a user exists with login: "andyman", email: "andy@overlord.com"
    And person "andy" exists with user: that user, name: "Andy Gibson", public_profile: false
    And a mindapple "mindapple1" exists with suggestion: "wrestling with bears", person: person "andy"
